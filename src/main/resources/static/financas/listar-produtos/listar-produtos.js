/* LISTAR-PRODUTOS.JS — INTEGRADO COM SPRING BOOT */
...
// VARIÁVEIS DE CONTROLE DOS DADOS REAIS
let transacoes = [];
let tipoAtivo  = "todos";
...
// FILTROS DE INTERFACE (ENTRADAS / SAÍDAS)
...
// Troca as datas de ordem automaticamente caso o usuário coloque "De" posterior a "Até"
...
// FILTRAGEM DINÂMICA (TIPO + INTERVALO TEMPORAL)
...
function renderizar(lista) {
  ...
  container.innerHTML = "";
  lista.forEach(t => {
    const tipoFormatado = t.tipo.toLowerCase(); // Tratamento do ENTRADA/SAIDA do Java
...
async function carregarTransacoesDaAPI() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      // FILTRO: Mapeia estritamente os lançamentos da categoria COSMETICO
      transacoes = todosLancamentos.filter(l => l.categoria === "COSMETICO");

      console.log("Lançamentos de produtos carregados do MySQL:", transacoes.length);
      aplicar();
    } else {
