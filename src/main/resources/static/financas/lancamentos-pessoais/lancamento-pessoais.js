/* FINANCAS-PESSOAIS.JS — INTEGRADO COM SPRING BOOT */
...
function abrirPopup(tipo) {
  ...
  tipoAtual = tipo; // Guarda se é "entrada" ou "saida"
...
async function confirmar() {
  ...
  // Monta o payload combinando com as regras do seu LancamentoFinanceiro.java
  const lancamentoDTO = {
    descricao: notas || "Lançamento Pessoal Manual",
    valor: valorInput,
    tipo: tipoAtual.toUpperCase(), // Envia "ENTRADA" ou "SAIDA" em maiúsculo para o Enum
    categoria: "PESSOAL"           // Categoria fixa baseada na pasta atual
  };
  ...
  try {
    ...
    if (response.ok) {
      ...
      const msgSucesso = tipoAtual === "entrada" ? "✅ Entrada registrada no banco!" : "✅ Saída registrada no banco!";
    ...
    console.error("Erro no POST financeiro:", error);
