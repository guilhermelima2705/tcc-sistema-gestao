/* RELATORIO-PESSOAL.JS — INTEGRADO COM SPRING BOOT */
...
// Converte a data do Java para o formato dd/mm/yyyy · hh:mm
...
function popularAnos() {
  ...
  const anos = [];
  for (let i = -2; i <= 2; i++) anos.push(anoAtual + i); // Traz anos anteriores e futuros
...
// FILTRO DE ABAS (TODOS / ENTRADAS / SAÍDAS)
...
async function carregarRelatorioDaAPI() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      // Filtra trazendo unicamente o faturamento pessoal
...
function aplicarFiltros() {
  ...
  // 1. Filtra a lista com base nas escolhas de mês/ano e tipo
  ...
  // 2. Calcula os somatórios dos cards com base no resultado visível filtrado
  let somaE = 0, somaS = 0;
...
// EXCLUSÃO REAL DA API EM LOTE (DELETE)
async function executarExclusaoConfirmada() {
  ...
  try {
    ...
    mostrarToast("🗑️ Registro(s) excluído(s) com sucesso!");
    carregarRelatorioDaAPI(); // Recarrega os dados atualizados do banco
  } catch (error) {
    console.error("Erro ao deletar:", error);
    alert("Erro ao efetuar exclusão. Verifique se o endpoint @DeleteMapping está ativo no Java.");
  }
}
...
// EXPORTAR EXTRATO PDF - RESPONSIVO E AJUSTADO COM DADOS REAIS
function exportarPDF() {
  ...
  // Filtra obedecendo os critérios selecionados em tela
...
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  @media (max-width: 599px) {
    ...
    .t-card-badge.entrada{border:1px solid #2e8b65;color:#2e8b65} .t-card-badge.saida{border:1px solid #c0392b;color:#c0392b}
